...
"""
propose_v9.py — 基於 propose_v7.py 完整重寫瓶頸偵測核心
...
class FlinkPropose:
    ...
    def detect_bottleneck(self):
        ...
        # Phase 1：TM 資源狀態（網路 + CPU，兩層共用）
        tm_out_saturated, saturation_rates, tm_cpu_sat_rates, cpu_sat_threshold =             self._compute_tm_network_saturation(task_info, subtask_locations)
        ...
        explicit_set = self._detect_explicit(
            task_info, operator_groups, ordered_operators, subtask_locations,
            tm_out_saturated, saturation_rates, tm_cpu_sat_rates, cpu_sat_threshold
        )
    ...
    def _compute_tm_network_saturation(self, task_info, subtask_locations):
        """
        計算各 TM 的網路飽和率與 CPU 飽和率，均以 IQR Tukey fence 動態閾值判定。

        網路飽和率設計（準則一）：
          IQR Tukey fence（Tukey 1977），上界 0.95（近飽和前兆），
          下界 0.85（TCP 在 85% 時因 ACK 延遲出現非線性衰退，
          Intel 網路效能白皮書建議保留 15% 緩衝）。

        CPU 飽和率設計（準則一）：
          tm_cpu_sat = Σ T_busy（同 TM 所有 subtask）/ cpu_cores
          超過 1.0 代表 TM 上的執行緒需求已超過實體核心數（需等待排程）。
          IQR Tukey fence 識別叢集中 CPU 壓力異常高的 TM。
          上界 min(Q3+1.5×IQR, 2.0)：雙核 TM 若 tm_cpu_sat>2.0 必然過載。
          下界 max(above, 0.7)：TM CPU 70% 利用率以下不視為飽和。

        回傳：
          tm_out_saturated  : { tm_id: bool }   網路已達嚴格閾值
          saturation_rates  : { tm_id: float }  網路飽和率（0~1+）
          tm_cpu_sat_rates  : { tm_id: float }  CPU 飽和率（0~2+）
          cpu_sat_threshold : float             CPU IQR 動態閾值
        """
        # ── 網路：彙總各 TM 出口流量 ──────────────────────────────────────────
        tm_traffic = {}
        for sid, info in task_info.items():
        ...
        if len(saturation_rates) >= 2:
            ...
            iqr = q3 - q1
            net_threshold = max(min(q3 + 1.5 * iqr, 0.95), 0.85) if iqr > 0 else 0.90
        else:
            net_threshold = 0.90

        tm_out_saturated = {}
        print(f"\n📊 [TM 網路狀態] 飽和閾值={net_threshold:.2%}:")
        for tm_id, rate in saturation_rates.items():
            limit = self.tm_bandwidth_map.get(tm_id, self.default_bandwidth)
            tm_out_saturated[tm_id] = (rate >= net_threshold)
            icon = "🔴" if tm_out_saturated[tm_id] else "🟢"
            print(f"   {tm_id}: {tm_traffic[tm_id]['out']/1e6:.2f}MB/s {icon} "
                  f"(Limit: {limit/1e6:.1f}MB/s, NetSat={rate:.2%})")

        # ── CPU：彙總各 TM 上所有 subtask 的 T_busy 總和 ──────────────────────
        # tm_cpu_sat = Σ T_busy / cpu_cores
        # 物理意義：TM 上所有執行緒需要的 CPU 時間 vs 實際擁有的核心數
        # 超過 1.0 代表需求超過供給，subtask 需等待核心排程
        tm_total_busy = {}
        for sid, info in task_info.items():
            tm_id = subtask_locations.get(sid, "unknown")
            if tm_id == "unknown":
        ...
        for tm_id, total_busy in tm_total_busy.items():
            cpu_cores = self.tm_bandwidth_map.get(tm_id + "_cpu",
                                                  next((v for k, v in vars(self).items()
                                                        if "cpu_capacity" in k.lower()
                                                        and tm_id in str(k)), None)) or 1.0
            # 直接從 cpu_capacity_map 取（定義在 get_taskmanager_info 內，這裡內聯）
            _cpu_cap = {"tm_20c_1": 2.0, "tm_20c_2": 2.0, "tm_20c_3": 2.0,
                        "tm_20c_4": 1.0, "tm_20c_5": 1.0}
            cpu_cores = _cpu_cap.get(tm_id, 1.0)
            tm_cpu_sat_rates[tm_id] = total_busy / cpu_cores if cpu_cores > 0 else total_busy

        # CPU IQR fence
        if len(tm_cpu_sat_rates) >= 2:
            cvals = sorted(tm_cpu_sat_rates.values())
            cq1 = _quantile_linear(cvals, 0.25)
            cq3 = _quantile_linear(cvals, 0.75)
            ciqr = cq3 - cq1
            cpu_sat_threshold = max(min(cq3 + 1.5 * ciqr, 2.0), 0.7) if ciqr > 0 else 0.8
        else:
            cpu_sat_threshold = 0.8

        print(f"\n📊 [TM CPU 狀態] 飽和閾值={cpu_sat_threshold:.2%}:")
        for tm_id, rate in tm_cpu_sat_rates.items():
            icon = "🔴" if rate >= cpu_sat_threshold else "🟢"
            print(f"   {tm_id}: CpuSat={rate:.2%} {icon} "
                  f"(Threshold={cpu_sat_threshold:.2%})")

        return tm_out_saturated, saturation_rates, tm_cpu_sat_rates, cpu_sat_threshold
    ...
    def _detect_explicit(self, task_info, operator_groups, ordered_operators,
                         ...
                         tm_cpu_sat_rates, cpu_sat_threshold):
        """
        顯性瓶頸偵測：兩個判定，均結合 TM 視角與 subtask 視角。

        ── 1. 網路瓶頸 → NETWORK_BOTTLENECK（合併原 1a + 1b）────────────────
        原先分成 NETWORK_TM_SATURATED（1a）和 NETWORK_BOTTLENECK（1b）：
          1a 只看 TM 整體頻寬是否打滿（嚴格 IQR fence）
          1b 只看 subtask outPool 積壓（排除受害者後）
        問題：兩者偵測同一件事（「這個 subtask 的輸出受到網路資源限制」），
              但用了不同原因碼和不同 d_overload 公式，量綱不統一。

        合併後統一判定邏輯（ANY 成立即為 NETWORK_BOTTLENECK）：
          N1：TM 整體頻寬已達 IQR fence（原 1a，嚴格物理上限）
          N2：outPool > 1.0（借 floating buffer）AND tm_sat > 叢集中位數
              （TM 有頻寬壓力但未達嚴格閾值，局部積壓已開始）
          N3：outPool ≥ 0.7 AND 通過受害者篩選
              （輸出緩衝積壓且不是被下游施壓造成的）

        受害者篩選（condA or condC）and not condD：
          condA：下游 max_inPool > 0.5 → 下游輸入積壓，本 subtask 是受害者
          condC：T_bp > T_busy × 2 → 等待遠超處理，自身資源充裕
          condD（否定條件）：outPool > 1.0 AND tm_sat > 中位數
                             → TM 網路積壓造成 bp，不能算受害者

        d_overload = max(tm_sat, min(out_pool, 1.5) / 1.5)
          量綱統一後取 max，反映 TM 頻寬壓力與 subtask 輸出積壓程度中較嚴重的那個。
          tm_sat 上限 1.0（物理頻寬），out_pool 上限夾在 1.5（floating buffer 近似上限），
          兩者歸一化後可以直接比較大小。

        ── 2. CPU 瓶頸 → CPU_BOTTLENECK（TM 視角 + subtask Z-score）──────────
        原先只用全局 Z-score，問題：
          Sink（busy=0ms）拉低 μ，導致雙核 TM 上 busy=550ms 的 subtask
          Z_busy 就超過 1.0，但每核利用率只有 27%，根本不是 CPU 瓶頸。

        修正後加入 TM CPU 視角：
          Z_busy > 1.0 AND T_busy > T_bp（subtask 層級：忙碌異常且非受害者）
          AND tm_cpu_sat > cpu_sat_threshold（TM 層級：TM 整體 CPU 確實飽和）
          兩個視角同時成立才命中，排除「TM 很閒但個別 subtask 相對較忙」的假陽性

        d_overload = max(T_busy, u_eff)
          T_bp=0 時 u_eff=T_busy，兩者相同；
          T_bp>0 時 u_eff 排除反壓時間，反映有機會執行時的真實利用率。

        回傳：[(subtask_id, cause, d_overload), ...]
        """
        # ── 前置統計量 ───────────────────────────────────────────────────────
        # Z-score 統計（全局）：用於 CPU 瓶頸的 subtask 層級異常偵測
        ...
        # 網路飽和率統計（叢集中位數）：用於 condD 和 N2 的 TM 網路壓力判定
        sat_vals   = sorted(saturation_rates.values())
        median_sat = statistics.median(sat_vals) if sat_vals else 0.0

        # 下游算子對照表（用 ordered_operators 保證順序穩定）
        downstream_of = {ordered_operators[i]: ordered_operators[i + 1]
                         for i in range(len(ordered_operators) - 1)}

        print(f"\n  [Z-score 統計量] μ_busy={mu_busy*1000:.1f}ms  σ_busy={sigma_busy*1000:.1f}ms")
        print(f"  [網路中位數] median_sat={median_sat:.2%}")
        print(f"  [CPU 飽和閾值] cpu_sat_threshold={cpu_sat_threshold:.2%}")
        ...
        for sid, info in task_info.items():
            ...
            # ── 1. 統一網路瓶頸判定 ──────────────────────────────────────────
            # outPool ≥ 0.7 才進入判定（TCP 流控在 buffer 70% 時出現 ACK 延遲，
            # RFC 3517 接收窗口機制；同時也是 Flink outPool 開始積壓的門檻）
            # N1 不需要 outPool 觸發，TM 整體飽和本身就足夠
            N1 = tm_out_saturated.get(tm_id, False)

            if N1 or out_pool >= 0.7:
                # 受害者篩選（只在 outPool ≥ 0.7 時才做，N1 直接跳過）
                is_victim = False
                ds_max_inpool = ds_max_busy_val = 0.0

                if not N1 and out_pool >= 0.7:
                    downstream_op = downstream_of.get(op_name)
                    if downstream_op:
                        ds_infos = [task_info[dst]
                                    for dst in operator_groups.get(downstream_op, [])
                                    if dst in task_info]
                        if ds_infos:
                            ds_max_inpool   = max(d["in_pool_usage"] for d in ds_infos)
                            ds_max_busy_val = max(d["T_busy"]        for d in ds_infos)

                            # condA：下游輸入積壓（下游消化不了）
                            condA = ds_max_inpool > 0.5
                            # condC：等待時間遠超處理時間（自身資源充裕，是受害者特徵）
                            condC = T_bp > T_busy * 2.0
                            # condD（否定條件）：outPool 借用 floating buffer 且 TM 網路有壓力
                            #   代表高 T_bp 是 network-induced bp（TM 出口積壓），不是下游施壓
                            #   此時 condC 即使成立也不應判受害者
                ...
                if is_victim:
                    ...
                    print(f"  [VICTIM] {sid}: outPool={out_pool:.2f}，"
                          f"ds_max_inPool={ds_max_inpool:.2f}，"
                          f"T_bp={T_bp*1000:.0f}ms/T_busy={T_busy*1000:.0f}ms，為受害者")
                    continue

                # 通過受害者篩選（或 N1 直接觸發）→ 判定為網路瓶頸
                # N2：outPool 借用 floating buffer + TM 頻寬相對偏高
                N2 = out_pool > 1.0 and tm_sat > median_sat
                # N3：outPool 積壓且通過受害者篩選
                N3 = out_pool >= 0.7

                if N1 or N2 or N3:
                    # d_overload：量綱統一後取 max
                    #   tm_sat：TM 頻寬壓力（0~1+，1.0 = 頻寬打滿）
                    #   out_pool_norm：subtask 輸出積壓程度（歸一化 0~1）
                    #     min(out_pool, 1.5) / 1.5：夾住在 floating buffer 上限 1.5
                    #     1.5 的依據：Flink 預設 floating buffer pool 約等於 exclusive buffer
                    #                 的 50%，exclusive+floating 合計上限約 1.5
                    #   取 max 代表「TM 和 subtask 兩個維度中較嚴重的那個」
                    out_pool_norm = min(out_pool, 1.5) / 1.5
                    d_overload    = max(tm_sat, out_pool_norm)
                    # 回存到 task_info 供 get_prioritized_list 直接使用，避免重複計算
                    info["_net_d_overload"] = d_overload

                    reason = ("N1:TM整體飽和" if N1 else
                              "N2:floating借用+TM有壓力" if N2 else
                              "N3:outPool積壓")
                ...
                continue  # 無論 N1/N2/N3 或 is_victim，都已處理完畢

            # ── 2. CPU 瓶頸判定（TM 視角 + subtask Z-score）─────────────────
            # 必須同時滿足：
            #   subtask 層級：Z_busy > 1.0（全局統計異常）且 T_busy > T_bp（真正在忙）
            #   TM 層級：tm_cpu_sat > cpu_sat_threshold（TM CPU 確實飽和）
            # 兩個視角同時成立才能確認是 CPU 瓶頸，排除「TM 很閒但 subtask 相對較忙」
            if sigma_busy >= SIGMA_MIN:
                Z_busy      = (T_busy - mu_busy) / sigma_busy
                tm_cpu_sat  = tm_cpu_sat_rates.get(tm_id, 0.0)
                if Z_busy > Z_THRESHOLD and T_busy > T_bp and tm_cpu_sat > cpu_sat_threshold:
                    free_time  = max(1.0 - T_bp, 0.001)
                    u_eff      = min(T_busy / free_time, 1.0)
                    d_overload = max(T_busy, u_eff)
                    explicit_set.append((sid, "CPU_BOTTLENECK", d_overload))
                    print(f"  [CPU] {sid}: Z_busy={Z_busy:.2f}>{Z_THRESHOLD}，"
                          f"T_busy={T_busy*1000:.0f}ms>T_bp={T_bp*1000:.0f}ms，"
                          f"tm_cpu_sat={tm_cpu_sat:.2%}>{cpu_sat_threshold:.2%}")
    ...
    def _build_report(self, task_info, operator_groups, ordered_operators, all_bottlenecks):
        ...
        for op_name in ordered_operators:
            ...
            network_count = sum(
                1 for st in subtasks
                if task_info[st].get("bottleneck_cause") == "NETWORK_BOTTLENECK"
            )
    ...
    def get_prioritized_list(self, filtered_ids):
        ...
        D_overload 依原因類型（全部使用直接觀測指標，不依賴 actual_input_rate）：
          NETWORK_BOTTLENECK → _net_d_overload（偵測時預存：max(tm_sat, out_pool_norm)）
          CPU_BOTTLENECK     → max(T_busy, u_eff)
          LATENT_*           → T_busy（不用 u_eff，避免高 T_bp 放大失真）
        """
        if not filtered_ids:
            return []
        ...
        for sid in filtered_ids:
            ...
            if cause == "NETWORK_BOTTLENECK":
                # 直接讀取偵測時預存的值，保證偵測與排序計算一致
                # 公式：max(tm_sat, min(out_pool, 1.5) / 1.5)
                d_overload = info.get("_net_d_overload", min(out_pool, 1.5) / 1.5)
            elif cause == "CPU_BOTTLENECK":
                d_overload = max(T_busy, u_eff)
            else:
                # LATENT_FLOW / LATENT_CPU / LATENT_SKEW
                # 用 T_busy 而非 u_eff：高反壓時 u_eff 趨近 1.0，導致排序倒置。
                d_overload = T_busy
    ...
    def auto_detect_and_migrate(self):
        ...
        for subtask_id, cause_str, score in self._bottleneck_subtasks:
            ...
            icon  = "🔥" if "CPU" in cause else ("🌐" if cause == "NETWORK_BOTTLENECK" else "⚠️")
